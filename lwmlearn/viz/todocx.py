...
def report_config(item_list, report_design, filename='report_config.xlsx', 
                  search_path=['graph_output', 'table_output'], **kwargs):
    ...
    **kwargs:
    ...
    df_list = [_report2df(item_list), _report2df(report_design), df_kwargs,
               df_search]
...
def _report2df(rep_item_list):
    ...
    for item in rep_item_list:
        new_list.append(
            [item[0], item[1], item[2], item[3], [i[0] for i in item[-1]], ]
            )
    df_item_list = pd.DataFrame(new_list, 
                                columns=['key_name', 'output_bool', 
                                         'txt_above', 'txt_below', 
                                         'subitems'])
    return df_item_list
